  ...
  experiments: [
    {
      ...
      content: {
        ...
        pretest: [
          { question: "In a singly linked list what does the next pointer of the last node contain?", options: ["The address of the first node", "-1", "NULL", "The address of the head pointer"], answerIndex: 2 },
          { question: "What is the time complexity of inserting a node at the beginning of a singly linked list?", options: ["O(n)", "O(n²)", "O(log n)", "O(1)"], answerIndex: 3 },
          { question: "Which C function is used to dynamically allocate memory for a new linked list node?", options: ["alloc()", "new()", "malloc()", "create()"], answerIndex: 2 },
          { question: "What happens to the head pointer when a node is inserted at the beginning of the list?", options: ["It remains pointing to the old first node", "It becomes NULL", "It is updated to point to the new node", "It points to the last node"], answerIndex: 2 },
          { question: "What must be done after deleting a node from a linked list to prevent memory leaks?", options: ["Set the deleted node's data to 0", "Call free() on the deleted node pointer", "Set the deleted node's next to NULL", "Reassign head to the deleted node"], answerIndex: 1 }
        ],
        procedure: [
        ...
        posttest: [
          { question: "To insert a node after a given node P in a singly linked list, which sequence of pointer assignments is correct?", options: ["P->next = newNode; newNode->next = P->next;", "newNode->next = P->next; P->next = newNode;", "newNode->next = P; P->next = newNode;", "head = newNode; newNode->next = P;"], answerIndex: 1 },
          { question: "What is the time complexity of deleting a node from the end of a singly linked list of n nodes?", options: ["O(1)", "O(log n)", "O(n)", "O(n²)"], answerIndex: 2 },
          { question: "A singly linked list has nodes A -> B -> C -> D -> NULL. After deleting node C what does B->next point to?", options: ["NULL", "A", "C (still — dangling pointer)", "D"], answerIndex: 3 },
          { question: "Which of the following is a disadvantage of singly linked list over arrays?", options: ["Fixed size at compile time", "O(n) insertion at beginning", "O(n) access by index — no direct random access", "Difficult to insert at end"], answerIndex: 2 },
          { question: "When inserting at position 3 in a list of 5 nodes what is the node you must traverse to before performing the insertion?", options: ["Node at position 1", "Node at position 2", "Node at position 3", "Node at position 4"], answerIndex: 1 }
        ],
        references: [
    ...
    },
    {
      ...
      content: {
        ...
        pretest: [
          { question: "How many pointer variables are used in the iterative linked list reversal algorithm?", options: ["1", "2", "3", "n (one per node)"], answerIndex: 2 },
          { question: "What is the initial value of the prev pointer at the start of iterative linked list reversal?", options: ["head", "head->next", "The last node", "NULL"], answerIndex: 3 },
          { question: "Why must the next pointer be saved before reversing cur->next?", options: ["To update the head pointer correctly", "To prevent losing the reference to the rest of the list since cur->next is about to be overwritten", "To check if the list has only one node", "To compute the length of the list"], answerIndex: 1 },
          { question: "After the iterative reversal loop completes, which pointer variable points to the new first node?", options: ["cur — it points to the last processed node", "next — it holds the original head", "prev — it is the last node processed and becomes the new head", "head — it is automatically updated"], answerIndex: 2 },
          { question: "What is the space complexity of iterative linked list reversal?", options: ["O(n)", "O(log n)", "O(n²)", "O(1)"], answerIndex: 3 }
        ],
        procedure: [
        ...
        posttest: [
          { question: "After iteratively reversing the list 1 -> 2 -> 3 -> 4 -> 5, what does node(1)->next point to?", options: ["node(2)", "node(5)", "NULL", "head"], answerIndex: 2 },
          { question: "For a list of n nodes how many iterations does the iterative reversal loop perform?", options: ["n-1", "n", "n+1", "n/2"], answerIndex: 1 },
          { question: "What is the state of the list after 2 iterations of iterative reversal on 1 -> 2 -> 3 -> 4 -> NULL?", options: ["NULL <- 1 <- 2   3 -> 4 -> NULL", "4 -> 3 -> 2 -> 1 -> NULL", "NULL <- 1   2 -> 3 -> 4 -> NULL", "2 -> 1 -> NULL   3 -> 4 -> NULL"], answerIndex: 0 },
          { question: "What happens when iterative reversal is applied to a list with only one node?", options: ["The program crashes because prev is NULL", "The loop executes once and correctly returns head pointing to the same single node", "The node's next pointer becomes the address of head", "head becomes NULL"], answerIndex: 1 },
          { question: "Why is iterative reversal preferred over using an auxiliary array for reversing a linked list?", options: ["Iterative reversal is always faster in terms of time complexity", "Iterative reversal achieves O(1) space complexity — no extra memory proportional to list size is needed unlike the auxiliary array approach which requires O(n) space", "Iterative reversal works on unsorted lists while array approach requires sorted input", "The auxiliary array approach modifies the original list structure"], answerIndex: 1 }
        ],
        references: [
          "Weiss M.A., Data Structures and Algorithm Analysis in C, 2nd Edition, Pearson",
          "Horowitz E., Sahni S., and Anderson-Freed S., Fundamentals of Data Structures in C, Silicon Press, 2008",
    ...
    },
    {
      ...
      content: {
        ...
        pretest: [
          { question: "What is the base case for recursive linked list reversal?", options: ["When the current node's data equals 0", "When head is NULL or head->next is NULL — return head", "When the list has more than one node", "When the recursive call depth exceeds n"], answerIndex: 1 },
          { question: "During recursive reversal of 1 -> 2 -> 3 -> NULL which node does the deepest recursive call return as newHead?", options: ["Node containing 1", "Node containing 2", "Node containing 3", "NULL"], answerIndex: 2 },
          { question: "What does head->next->next = head accomplish during the unwinding phase of recursive reversal?", options: ["It moves head to the next node", "It makes the node after head point back to head — reversing that link", "It sets the new head pointer", "It frees the current node"], answerIndex: 1 },
          { question: "What is the space complexity of recursive linked list reversal and why?", options: ["O(1) — only three pointers are used", "O(n) — the call stack holds one frame for each of the n nodes", "O(log n) — recursion halves the problem at each step", "O(n²) — two recursive calls are made per node"], answerIndex: 1 },
          { question: "Why must head->next be set to NULL after reversing the link in the recursive approach?", options: ["To free the current node's memory", "Because the original first node becomes the last node and must point to NULL to terminate the list", "To prevent the function from recursing indefinitely", "Because NULL signals the base case for the next recursive call"], answerIndex: 1 }
        ],
        procedure: [
        ...
        posttest: [
          { question: "For a list of 5 nodes how deep does the recursive call stack grow during recursive reversal?", options: ["2", "3", "4", "5"], answerIndex: 3 },
          { question: "After recursive reversal of 10 -> 20 -> 30 -> 40 -> NULL what does node(10)->next equal?", options: ["node(20)", "node(40)", "NULL", "node(30)"], answerIndex: 2 },
          { question: "What is the primary disadvantage of recursive linked list reversal over iterative reversal for very large lists?", options: ["Recursive reversal produces incorrect results for large lists", "Recursive reversal uses O(n) call stack space and risks stack overflow for very large n", "Recursive reversal has higher time complexity O(n²)", "Recursive reversal requires extra heap memory proportional to n"], answerIndex: 1 },
          { question: "In the recursive reversal what does newHead always contain throughout all the unwinding frames?", options: ["The address of the current frame's head node", "NULL until the last frame", "The address of the original last node which becomes the new first node", "The address of the original first node"], answerIndex: 2 },
    ...
    },
    {
      ...
      content: {
        ...
        pretest: [
          { question: "In the slow and fast pointer technique for finding the middle of a linked list, how many steps does the fast pointer move per iteration?", options: ["1", "2", "3", "n/2"], answerIndex: 1 },
          { question: "What is the termination condition for the slow-fast pointer loop when finding the middle node?", options: ["When slow equals fast", "When slow reaches NULL", "When fast is NULL or fast->next is NULL", "When the count of steps equals n/2"], answerIndex: 2 },
          { question: "For a linked list with 7 nodes what is the position (1-indexed) of the middle node found by the slow-fast pointer technique?", options: ["3", "4", "5", "2"], answerIndex: 1 },
          { question: "What is the time complexity of finding the middle node using the slow-fast pointer technique?", options: ["O(n²)", "O(n log n)", "O(n/2) = O(n)", "O(1)"], answerIndex: 2 },
          { question: "Why is the slow-fast pointer technique preferred over the naive count-then-traverse approach for finding the middle?", options: ["It has lower time complexity O(log n)", "It finds the middle in a single traversal requiring O(1) space instead of two traversals", "It works on unsorted lists while the naive approach requires sorted input", "It modifies the list structure to make future middle finding O(1)"], answerIndex: 1 }
        ],
        procedure: [
        ...
        posttest: [
          { question: "For list 10 -> 20 -> 30 -> 40 -> 50 -> 60 -> NULL (6 nodes) where does the slow pointer stop when finding the middle?", options: ["Node 2 (value 20)", "Node 3 (value 30)", "Node 4 (value 40)", "Node 5 (value 50)"], answerIndex: 2 },
          { question: "A linked list has values [5, 2, 8, 1, 9, 3]. What is the output of the minimum finding traversal?", options: ["2", "3", "1", "5"], answerIndex: 2 },
          { question: "The slow-fast pointer technique is also known as:", options: ["Two-stack technique", "Divide and conquer pointer method", "Floyd's Tortoise and Hare algorithm", "Sentinel pointer method"], answerIndex: 2 },
