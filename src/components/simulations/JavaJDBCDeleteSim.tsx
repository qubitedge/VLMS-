...
import { Database, Trash2, ArrowRight, Play, RefreshCcw, Table } from 'lucide-react';

export function JavaJDBCDeleteSim() {
  ...
  const runDelete = () => {
    ...
    setTimeout(() => {
      setStep(2); // Database processing (target found)
      setTimeout(() => {
        setRows([{ id: 100, name: 'Alice' }]); // Raj deleted
    ...
    <div className="flex flex-col h-full bg-white rounded-lg shadow-sm border border-indigo-100 overflow-hidden text-slate-800 font-sans">
      <div className="bg-indigo-50 border-b border-indigo-100 p-4 flex items-center justify-between">
        <div className="flex items-center space-x-2 text-indigo-700">
          ...
          <h3 className="font-semibold text-lg">JDBC Delete (DML)</h3>
        </div>
                ...
                <AnimatePresence>
                  {step === 3 && (
                    <motion.div 
                      ...
                      transition={{ duration: 1, ease: "easeInOut" }}
                      className="absolute top-1/2 -translate-y-1/2 left-full ml-4 bg-red-500 text-white font-mono text-xs font-bold px-3 py-1 rounded-full shadow-xl z-30 flex items-center"
                    >
                    ...
                    <tbody>
                      {rows.map((row, idx) => (
                        <motion.tr 
                          ...
                          layout
                          initial={{ opacity: 1 }}
                          exit={{ opacity: 0, backgroundColor: '#ef4444', color: '#ffffff' }}
                          className={`border-b border-slate-100 last:border-0 ${step === 2 && row.id === 101 ? 'bg-red-100 text-red-900 border-red-300' : ''}`}
                        >
                          <td className="px-3 py-2 border-r border-slate-100 font-bold">{row.id}</td>
                          <td className="px-3 py-2">{row.name}
                            {step === 2 && row.id === 101 && <span className="ml-2 text-red-500 font-bold text-[10px] uppercase">Target found</span>}
                          </td>
                        </motion.tr>
                      ))}
                    </tbody>
        ...
        <div className="w-full lg:w-1/3 flex flex-col space-y-4">
          ...
          <div className="flex-1 bg-slate-900 border border-slate-700 rounded-lg p-6 font-mono text-xs text-slate-300 overflow-y-auto">
            ...
            <div className="space-y-4">
              <div>
                <span className="text-blue-400">String</span> sql = <span className="text-green-300">"DELETE FROM student WHERE id=101"</span>;
              </div>
              ...
              <div className={`p-2 rounded border transition-colors duration-500 ${step >= 4 ? 'bg-green-900/30 border-green-500 text-white' : 'border-transparent opacity-50'}`}>
                ...
                <div className="ml-4">
                  System.out.println(<span className="text-green-300">"Row deleted! count = "</span> + count);
                </div>
                {'}'}
              </div>
